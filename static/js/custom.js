...
$(document).on("click", "#question-btn", function(){
    ...
    $(document).on('click', '#increase-btn', function () {
        ...
        console.log('quantity value',input_value);
    
        let shipping_amount = $(".product-shipping_amount-" + id).val();
        let product_processing_fee = $(".product-product_processing_fee-" + id).val();
        let product_tax_fee = $(".product-product_tax_fee-" + id).val();
        let product_stock_qty = $(".product-product_stock_qty-" + id).val();
        ...
        if (input_value > parseInt(product_stock_qty)) {
            Swal.fire({
                icon: 'warning',
                title: 'Oops... Stock Qty Exceeded!',
                text: 'You are exceeding the stock current quantity of ' + product_stock_qty,
            });
            $("#qty" + id).val(product_stock_qty);
        } else {
            // Ajax to update cart
            $.ajax({
                url: '/ajax/update-cart/',
    ...
    $(document).on('click', '#decrease-btn', function () {
        ...
        if (input_value > 1) {
            ...
            // Don't allow to decrease quantity below 1
            $("#qty" + id).val(1);
        }
